...
class CVAE_3D(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv1 = nn.Conv3d(1, 16, 3, stride=2, padding=1)               # 16 x 40 x 48 x 40
        self.conv2 = nn.Conv3d(16, 32, 3, stride=2, padding=1)              # 32 x 20 x 24 x 20
        self.conv3 = nn.Conv3d(32, 64, 3, stride=2, padding=1)              # 64 x 10 x 12 x 10
        self.conv4 = nn.Conv3d(64, 64, 3, stride=2, padding=1)              # 64 x 5 x 6 x 5
        self.bn1 = nn.BatchNorm3d(16)
        self.bn2 = nn.BatchNorm3d(32)
        self.bn3 = nn.BatchNorm3d(64)
        self.bn4 = nn.BatchNorm3d(64)
        self.fc10 = nn.Linear(9600, Settings().dimension)
        self.fc11 = nn.Linear(9600, Settings().dimension)

        self.fc3 = nn.Linear(Settings().dimension,1200)
        self.upconv1 = nn.ConvTranspose2d(8, 64, 3, stride=2, padding=1, output_padding=1)    # 64 x 10 x 12 x 10 
        self.upconv2 = nn.ConvTranspose2d(64, 64, 3, stride=2, padding=1, output_padding=1)   # 64 x 20 x 24 x 20 
        self.upconv3 = nn.ConvTranspose2d(64, 32, 3, stride=2, padding=1, output_padding=1)   # 32 x 40 x 48 x 40 
        self.upconv4 = nn.ConvTranspose2d(32, 1, 3, stride=2, padding=1, output_padding=1)    # 1 x 80 x 96 x 80
    ...
    def encoder(self, image):
        ...
        mu = torch.tanh(self.fc10(h4.flatten(start_dim=1)))
        logVar = self.fc11(h4.flatten(start_dim=1))
        return mu, logVar

    def decoder(self, encoded):
        h5 = F.relu(self.fc3(encoded)).reshape([encoded.size()[0], 8, 5, 6, 5])
