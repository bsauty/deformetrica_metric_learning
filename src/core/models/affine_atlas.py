...
class AffineAtlas(AbstractStatisticalModel):
    ...
    def compute_log_likelihood(self, dataset, population_RER, individual_RER, mode='complete', with_grad=False):
        ...
        template_points = {key: self.dataset.tensor_scalar_type(value) for key, value in self.template.get_points().items()}
        template_data = {key: self.dataset.tensor_scalar_type(value) for key, value in self.template.get_data().items()}
        ...
        if with_grad:
            ...
            for i, target in enumerate(targets):
                translation_vector_i = self.dataset.tensor_scalar_type(translation_vectors[i]).requires_grad_(with_grad and not self.is_frozen['translation_vectors'])
                rotation_angles_i = self.dataset.tensor_scalar_type(rotation_angles[i]).requires_grad_(with_grad and not self.is_frozen['rotation_angles'])
                scaling_ratio_i = self.dataset.tensor_scalar_type([scaling_ratios[i]]).requires_grad_(with_grad and not self.is_frozen['scaling_ratios'])
            ...
            for i, target in enumerate(targets):
                translation_vector_i = self.dataset.tensor_scalar_type(translation_vectors[i]).requires_grad_(with_grad)
                rotation_angles_i = self.dataset.tensor_scalar_type(rotation_angles[i]).requires_grad_(with_grad)
                scaling_ratio_i = self.dataset.tensor_scalar_type([scaling_ratios[i]]).requires_grad_(with_grad)
    ...
    def _rotate(self, rotation_angles, points):
        ...
        for key, value in points.items():
            if key == 'landmark_points':
                out[key] = torch.matmul(rotation_matrix.unsqueeze(0), value.unsqueeze(self.dataset.dimension - 1)).view(-1, self.dataset.dimension)
            elif key == 'image_points':
                raise RuntimeError('Not implemented yet.')
            else:
                raise RuntimeError('That\'s unexpected!')
    ...
    def _compute_rotation_matrix(self, rotation_angles):
        ...
        if self.dataset.dimension == 2:
        ...
        elif self.dataset.dimension == 3:  # Using Euler angles: https://fr.wikipedia.org/wiki/Matrice_de_rotation
            psi = rotation_angles[0]
            theta = rotation_angles[1]
            phi = rotation_angles[2]

            rot_x = torch.zeros(3, 3).type(self.dataset.tensor_scalar_type)
            rot_x[0, 0] = phi.cos()
            rot_x[0, 1] = - phi.sin()
            rot_x[1, 0] = phi.sin()
            rot_x[1, 1] = phi.cos()
            ...
            rot_y = torch.zeros(3, 3).type(self.dataset.tensor_scalar_type)
            rot_y[0, 0] = theta.cos()
            rot_y[0, 2] = theta.sin()
            rot_y[2, 0] = - theta.sin()
            rot_y[2, 2] = theta.cos()
            ...
            rot_z = torch.zeros(3, 3).type(self.dataset.tensor_scalar_type)
            rot_z[1, 1] = psi.cos()
            rot_z[1, 2] = - psi.sin()
            rot_z[2, 1] = psi.sin()
            rot_z[2, 2] = psi.cos()
    ...
    def _initialize_translation_vectors(self):
        ...
        if self.get_translation_vectors() is None:
            translation_vectors = np.zeros((self.number_of_subjects, self.dataset.dimension))
    ...
    def _initialize_rotation_angles(self):
        ...
        if self.get_rotation_angles() is None:
            rotation_angles = np.zeros((self.number_of_subjects, self.dataset.dimension))
    ...
    def _write_model_predictions(self, dataset, individual_RER, output_dir, compute_residuals=True):
        ...
        template_points = {key: dataset.tensor_scalar_type(value) for key, value in self.template.get_points().items()}
        template_data = {key: dataset.tensor_scalar_type(value) for key, value in self.template.get_data().items()}
        ...
        for i, (subject_id, target) in enumerate(zip(dataset.subject_ids, targets)):
            ...
            # Inverse deformation --------------------------------------------------------------------------------------
            target_points = {key: dataset.tensor_scalar_type(value) for key, value in target.get_points().items()}
            target_data = {key: dataset.tensor_scalar_type(value) for key, value in target.get_data().items()}
