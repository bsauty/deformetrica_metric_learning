...
class SpatiotemporalReferenceFrame:
    ...
    def __init__(self, exponential_factory):
        self.geodesic = Geodesic(exponential_factory)
    ...
    def get_position(self, time, sources):

        # Assert for coherent length of attribute lists.
        assert len(self.positions_t) == len(self.projected_modulation_matrix_t) == len(self.times)
        ...
        if len(self.times) == 1:
            ...
            self.exponential.set_initial_position(self.template_data_t[0])
    ...
    def update(self):
        ...
        self.positions_t = self.geodesic._get_geodesic_trajectory()
