...
from pydeformetrica.src.support.utilities.kernel_types import TorchKernel
...
def OrientedSurfaceDistance(points1, points2, surf1, surf2, kernel_width=0.):
    ...
    """
    assert kernel_width>0, "Please set the kernel width in current distance computation"
    kernel = TorchKernel()
    ...
    def current_scalar_product(p1, p2, n1, n2):
        return torch.dot(n1.view(-1), kernel.Convolve(p2,n2,p1).view(-1))
    return current_scalar_product(c1, c1, n1, n1) + current_scalar_product(c2, c2, n2, n2) - 2*current_scalar_product(c1, c2, n1, n2)
